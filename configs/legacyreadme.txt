...
## GOAL : open5gs + istio + some dashboard + PacketRusher (or other possible simulators)
...
# and also look at : https://github.com/AIDY-F2N/OAI-UERANSIM
    - use configs/istio-install.yaml configuration file for istio installation
...
# Instead of ROOK-CEPH, just create PV manually
    ...
    - change if mongodb configuration is designating specific PV
...
# See ./dbcontrol for controlling subscription database 
    - currently using auto generated configurations (create_ues.py -> susbscribers.yaml)
...
# setup nodeport, using kc apply -f ./configs/amf_nodeport.yaml
    - if you are running packetrusher in bare metal
# for packetrusher config.yaml, 
    - change gNB's N2/3 ip to node's ip running packetrusher, 
    - and amfif to node's ip that running AMF's pod (and its port number, as you set in amf_nodeport.yaml)
    - and don't forget to match PLMN and Slice information with the core
    - and also UE's SIM credentials (OPc, key, imsi, etc)

### Open5GS configuration ###
    - using latest helm chart : helm pull oci://registry-1.docker.io/gradiant/open5gs --version 2.2.6
    - this helm chart uses open5gs 2.7.2, which is the latest version
    - amf configuration's config field should be modified to synchronize with UE sim's configuration
    - and nrf configuration's config field should be also modified 
    - currently turned on root privilege of AMF pod
