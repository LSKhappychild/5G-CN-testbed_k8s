...
- **Default Login Credentials**:
	...
	- this should reside inside open5gs/accounts collection
...
* See pcap log for further checking configurations
...
- Update the **AMF IP** (amfConfig's address field) in the UERANSIM gNB configuration if it changes.

- Adjust the UE configuration's `gnbSearchList` field accordingly.
...
Since officially PacketRusher does not support Docker, run it **outside** the Kubernetes cluster.

  

1. **Expose the AMF**:

- Create a NodePort service for the AMF (if you are running outside of cluster):
...
- **AMF IP/Port**: Ip should be the node's ip where AMF pod is running, and port should be the one set in nodeport service (see amf_nodeport.yaml)
...
- This chart deploys **Open5GS 2.7.2**, which is the latest version so far

- Adjust the `values.yaml` file for:

- **AMF Configuration**: Set parameters such as MCC, MNC, TAC, SST, etc.

- **NRF Configuration**: config field should be modified (plmn setting)
